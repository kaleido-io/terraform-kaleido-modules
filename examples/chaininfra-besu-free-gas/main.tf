...
resource "kaleido_platform_application" "application" {
  name = var.baf_policies[count.index].application
  count = length(var.baf_policies)
}

locals {
  baf_policies = [for index,policy in var.baf_policies : {
    application_id = kaleido_platform_application.application[index].id
    file = policy.file
  }]
}
